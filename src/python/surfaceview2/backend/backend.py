...
from ._common import _http_json_post, _upload_json_to_google_cloud
...
class AblyClientChannel:
    ...
    def _send_message_buffer(self):
        ...
        if self._parent._client:
            self._parent._client.publish(self._channel, json.dumps(
                {'messages': messages}).encode('utf-8'), qos=1)
...
class Backend:
    def __init__(self, *, google_bucket_name: str, app_url: str, label: str, admin_user_id: Union[str, None] = None):
        ...
        self._task_manager = TaskManager(
            on_publish_message=on_publish_message, google_bucket_name=google_bucket_name)
        self._subfeed_manager = SubfeedManager(
            on_publish_message=on_publish_message, google_bucket_name=google_bucket_name)
    ...
    def iterate(self):
        ...
        elapsed_since_last_report_alive = min(time.time(
        ) - self._last_report_alive_timestamp, time.time() - self._last_report_alive_timestamp)
        if elapsed_since_last_report_alive > 60:
            self._report_alive()

        # update authorized users
        elapsed_since_update_user_permissions = time.time(
        ) - self._last_update_user_permissions_timestamp
    ...
    def cleanup(self):
        ...
        self._subfeed_manager.cleanup()
        self._ably_client.cleanup()
    ...
    def _on_ably_message(self, message: dict):
        ...
        if type0 == 'initiateTask':
                ...
                if td is not None:
                    try:
                        ...
                        self._task_manager.add_task(
                            task_hash, task_data, taskjob)
                    except Exception as e:
                        msg = {'type': 'taskStatusUpdate', 'taskHash': task_hash,
                               'status': 'error', 'error': f'Unable to create job: {str(e)}'}
                        self._publish_to_task_status(msg)
                else:
                    msg = {'type': 'taskStatusUpdate', 'taskHash': task_hash, 'status': 'error',
                           'error': f'Unable to find task function: {function_id}'}
                    self._ably_client.publish(
                        self._registration['serverChannelName'], msg)
            ...
            if feed_id is not None and subfeed_hash is not None:
                self._subfeed_manager.subscribe_to_subfeed(
                    feed_id=feed_id, subfeed_hash=subfeed_hash)
    ...
    def _upload_config_object_to_google_cloud(self):
        ...
        _upload_json_to_google_cloud(
            self._google_bucket_name, self._config_object_name(), self._config_object)
    ...
    def _renew_registration(self):
        ...
        print(f'==========================================================================================')
        print(
            f'Backend URI: gs://{self._google_bucket_name}/{self._config_object_name()}')
        print(f'')
        ...
        self._ably_client.reconnect(
            client_channel_name=client_channel_name, token_details=token_details)
    ...
    def _user_can_append_to_subfeed(self, auth_user_id: Union[str, None], feed_id: str, subfeed_hash: str):
        ...
        if perm.get('appendToAllFeeds', False):
            return True
        f = perm.get('feeds', {})
        if feed_id in f:
            if f[feed_id].get('append', False):
                return True
        return False
    ...
    def _user_can_get_user_permissions(self, auth_user_id: Union[str, None], user_id: str):
        ...
        if auth_user_id == user_id:
            return True
        if self._user_is_admin(auth_user_id):
            return True
        return False
