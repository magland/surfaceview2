...
import { SampledSlice } from './mainLayer';
...
const SampledSlicesView: FunctionComponent<Props> = ({sampledSlices}) => {
    ...
    const {N1, N2}  = useMemo(() => {
        const N1 = sampledSlices ? sampledSlices[0].data[0].length : 0
        const N2 = sampledSlices ? sampledSlices[0].data[0][0].length : 0
        return {N1, N2}
    }, [sampledSlices])
            ...
            <div>
                {
                    sampledSlices ? (
                        <pre>
                            {sampledSlices?.length} slices ({N1} x {N2})<br />
